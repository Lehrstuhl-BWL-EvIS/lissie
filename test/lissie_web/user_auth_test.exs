defmodule LissieWeb.UserAuthTest do
  use LissieWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Lissie.Accounts
  alias Lissie.Accounts.Scope
  alias LissieWeb.UserAuth

  import Lissie.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, LissieWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      LissieWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        LissieWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
