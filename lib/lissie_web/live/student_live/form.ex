defmodule LissieWeb.StudentLive.Form do
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope}>
      ...
      <.form for={@form} id="student-form" phx-change="validate" phx-submit="save">
        ...
        <footer>
          ...
          <.button navigate={return_path(@current_scope, @return_to, @student)}>Cancel</.button>
        </footer>
      </.form>
    </Layouts.app>
    """
  ...
  defp apply_action(socket, :edit, %{"id" => id}) do
    ...
    |> assign(:form, to_form(Students.change_student(socket.assigns.current_scope, student)))
  end

  defp apply_action(socket, :new, _params) do
    student = %Student{user_id: socket.assigns.current_scope.user.id}
  ...
  def handle_event("validate", %{"student" => student_params}, socket) do
    changeset = Students.change_student(socket.assigns.current_scope, socket.assigns.student, student_params)
  ...
  defp save_student(socket, :edit, student_params) do
    case Students.update_student(socket.assigns.current_scope, socket.assigns.student, student_params) do
      {:ok, student} ->
        {:noreply,
         socket
         |> put_flash(:info, "Student updated successfully")
         |> push_navigate(
           to: return_path(socket.assigns.current_scope, socket.assigns.return_to, student)
         )}
  ...
  defp save_student(socket, :new, student_params) do
    case Students.create_student(socket.assigns.current_scope, student_params) do
      {:ok, student} ->
        {:noreply,
         socket
  ...
  defp return_path(_scope, "index", _student), do: ~p"/students"
  defp return_path(_scope, "show", student), do: ~p"/students/#{student}"
end
